...
- uruchamiać płatnej macierzy bez jawnego potwierdzenia ustawionego w środowisku bieżącego procesu oraz dwóch skończonych, ściśle dodatnich limitów: per model i całego przebiegu;
- zapisywać potwierdzenia płatnego przebiegu w `.env`; ma ono wygasać razem z poleceniem lub sesją powłoki;
