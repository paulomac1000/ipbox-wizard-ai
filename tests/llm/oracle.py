...
from python_helper.report_metadata import calculation_meta, source_coverage_complete
...
def compute_reference(scenario: dict[str, Any]) -> dict[str, Any]:
    ...
    base["calculation_meta"] = calculation_meta(input_data)
    ...
    warnings_seen = set(base.get("stops_reviews", {}).get("warnings", []))
    facts["asset_classification_requires_review"] = "ASSET_CLASSIFICATION_REVIEW" in warnings_seen
    facts["contribution_classification_requires_review"] = (
        "CONTRIBUTION_CLASSIFICATION_REVIEW" in warnings_seen
    )
