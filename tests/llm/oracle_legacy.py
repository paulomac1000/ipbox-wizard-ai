...
from python_helper.report_metadata import source_coverage_complete
...
def compute_reference(scenario: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        coverage_complete = source_coverage_complete(input_data)
    except ValueError as exc:
        raise ScenarioError(str(exc)) from exc
    decision_facts = {
        ...
        "no_qualifying_ip_income_after_complete_evidence": (
            coverage_complete and annual_ip_revenue == 0 and not other_stop_condition
        ),
        "source_coverage_incomplete": not coverage_complete,
    ...
    for raw_month in input_data.get("miesiace", []):
        ...
        for raw_cost in month.get("koszty", []):
            ...
            asset = cost.get("asset", {})
            if asset is None:
                asset = {}
            if not isinstance(asset, dict):
                raise ScenarioError("cost.asset must be a mapping")
            item = CostItem(
                ...
                cost_type=str(cost.get("cost_type", "ordinary")),
                asset_status=str(cost.get("asset_status", asset.get("status", "not_applicable"))),
                asset_treatment=str(cost.get("asset_treatment", asset.get("treatment", ""))),
                asset_evidence_ref=str(
                    cost.get("asset_evidence_ref", asset.get("evidence_ref", ""))
                ),
            )
            if not item.basket:
                item = classify_cost(item, social_in_kpir, health_in_kpir)
                if item.note.startswith("Potential contribution"):
                    warnings.add("CONTRIBUTION_CLASSIFICATION_REVIEW")
                    decision_facts["contribution_classification_requires_review"] = True
                if item.note.startswith("Potential asset"):
                    warnings.add("ASSET_CLASSIFICATION_REVIEW")
                    decision_facts["asset_classification_requires_review"] = True
