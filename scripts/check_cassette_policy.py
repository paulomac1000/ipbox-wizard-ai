...
def main(root: str | Path | None = None) -> int:
    try:
        effective_root = resolve_cassette_root(root)
    except ValueError as exc:
        print(f"Cassette policy configuration error: {exc}", file=sys.stderr)
        return 1
    ...
    if not effective_root.exists():
        ...
        return 0
    cassette_files = [
        path for path in effective_root.glob("*/*.yaml") if path.name != "_manifest.yaml"
    ]
    manifests = list(effective_root.glob("*/_manifest.yaml"))
    extras = unexpected_model_directories(effective_root)
    root_extras = unexpected_root_entries(effective_root)
    ...
    errors.extend(
        ...
        for error in validate_model(model, cassette_root=effective_root)
    )
    if errors:
        print(
    ...
    print(
        ...
        f"({len(BENCHMARK_MODELS) * scenario_count} cassettes) from {effective_root}"
    )
    return 0
...
def cli() -> int:
    ...
    parser.add_argument(
        "--cassette-root",
        help="Override VCR_CASSETTES_ROOT for this policy check.",
    )
    args = parser.parse_args()
    return main(args.cassette_root)
